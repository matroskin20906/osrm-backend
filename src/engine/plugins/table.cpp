        ...
        for (std::size_t row = 0; row < num_sources; row++)
        {
            for (std::size_t column = 0; column < num_destinations; column++)
            {
                    ...
                    (std::get<0>(result_tables_tuple))[table_index] == MAXIMAL_EDGE_DURATION)
                {
                    ...
                    {
                        (std::get<1>(result_tables_tuple))[table_index] = distance_estimate;
                    }
                ...
                if (params.scale_factor > 0 && params.scale_factor != 1 &&
                    (std::get<0>(result_tables_tuple))[table_index] != MAXIMAL_EDGE_DURATION &&
                    (std::get<0>(result_tables_tuple))[table_index] != 0)
                {
                    EdgeDuration diff =
                    ...
                    {
                        (std::get<0>(result_tables_tuple))[table_index] = MAXIMAL_EDGE_DURATION - 1;
                    }
                    else
                    {
                        (std::get<0>(result_tables_tuple))[table_index] = std::lround(
                            (std::get<0>(result_tables_tuple))[table_index] * params.scale_factor);
                    }
                }
            }
        }
